  ...
  io.on('connection', (socket) => {
    ...
    socket.on('sendMessage', async ({ recipientId, content, workspaceId, conversationId }) => {
      try {
        ...
        // Handle workspaceId that could be a populated object
        const wsId = typeof workspaceId === 'object' ? (workspaceId._id || workspaceId) : workspaceId;
        if (!recipientId || !wsId || !content?.trim()) return;
        if (!mongoose.Types.ObjectId.isValid(recipientId) || !mongoose.Types.ObjectId.isValid(wsId)) return;

        let conversation = await Conversation.findOne({ workspaceId: wsId, participants: { $all: [senderId, recipientId] } });
        if (!conversation) {
          conversation = await Conversation.create({ participants: [senderId, recipientId], workspaceId: wsId });
        }
        const newMessage = new Message({ conversationId: conversation._id, senderId, content: content.trim() });
